function H = univariate_gradu_gradv_nurbs(H, space, Tweights)
...
%         H, space, LEVEL, LEVEL_IND, KNOT_AREA, CUBOID_SPLINES_LEVEL, TWEIGHTS)
%
% Purpose
% -------
...
%
% space, LEVEL          Hierarchical space and the (global) level being assembled.
%
...
%                        (in space.knots{d}) over which to integrate.
%
...
%   H.stiffness.order when combining directions.
    ...
    for dim = 1:3
        ...
        for i = 1:9
            ...
            H.stiffness.K{dim}{i}(:) = {sparse(space.ndof_dir(dim), ...
                space.ndof_dir(dim))};
        end
        for l=1:length(space.knots{dim})-1
            if space.knots{dim}(l) < space.knots{dim}(l+1)
                ...
                quadValuesDeriv = evalNURBSDeriv(space.knots{dim}, space.degree(dim), Tweights{dim}', xx);
                if dim == 1
                    for i = l-space.degree(dim):l
                        for j = l-space.degree(dim):l
                            for r = 1:H.stiffness.R(1,dim)
                                H.stiffness.K{dim}{1}{r}(i,j)  = ...
                                    H.stiffness.K{dim}{1}{r}(i,j)  + ((b-a)/2)*sum(w.*quadValuesDeriv(i,:)'.*quadValuesDeriv(j,:)'.*quadValues2'*H.stiffness.SVDU{1}{dim}(:,r));
                            end
                            for r = 1:H.stiffness.R(2,dim)
                                H.stiffness.K{dim}{2}{r}(i,j)  = ...
                                    H.stiffness.K{dim}{2}{r}(i,j)  + ((b-a)/2)*sum(w.*quadValues(i,:)'.*quadValuesDeriv(j,:)'.*quadValues2'*H.stiffness.SVDU{2}{dim}(:,r));
                                H.stiffness.K{dim}{4}{r}(i,j)  = ...
                                    H.stiffness.K{dim}{4}{r}(i,j)  + ((b-a)/2)*sum(w.*quadValuesDeriv(i,:)'.*quadValues(j,:)'.*quadValues2'*H.stiffness.SVDU{2}{dim}(:,r));
                            end
                            for r = 1:H.stiffness.R(3,dim)
                                H.stiffness.K{dim}{3}{r}(i,j)  = ...
                                    H.stiffness.K{dim}{3}{r}(i,j)  + ((b-a)/2)*sum(w.*quadValues(i,:)'.*quadValuesDeriv(j,:)'.*quadValues2'*H.stiffness.SVDU{3}{dim}(:,r));
                                H.stiffness.K{dim}{7}{r}(i,j)  = ...
                                    H.stiffness.K{dim}{7}{r}(i,j)  + ((b-a)/2)*sum(w.*quadValuesDeriv(i,:)'.*quadValues(j,:)'.*quadValues2'*H.stiffness.SVDU{3}{dim}(:,r));
                            end
                            for r = 1:H.stiffness.R(4,dim)
                                H.stiffness.K{dim}{5}{r}(i,j)  = ...
                                    H.stiffness.K{dim}{5}{r}(i,j)  + ((b-a)/2)*sum(w.*quadValues(i,:)'.*quadValues(j,:)'.*quadValues2'*H.stiffness.SVDU{4}{dim}(:,r));
                            end
                            for r = 1:H.stiffness.R(5,dim)
                                H.stiffness.K{dim}{6}{r}(i,j)  = ...
                                    H.stiffness.K{dim}{6}{r}(i,j)  + ((b-a)/2)*sum(w.*quadValues(i,:)'.*quadValues(j,:)'.*quadValues2'*H.stiffness.SVDU{5}{dim}(:,r));
                                H.stiffness.K{dim}{8}{r}(i,j)  = ...
                                    H.stiffness.K{dim}{8}{r}(i,j)  + ((b-a)/2)*sum(w.*quadValues(i,:)'.*quadValues(j,:)'.*quadValues2'*H.stiffness.SVDU{5}{dim}(:,r));
                            end
                            for r = 1:H.stiffness.R(6,dim)
                                H.stiffness.K{dim}{9}{r}(i,j)  = ...
                                    H.stiffness.K{dim}{9}{r}(i,j)  + ((b-a)/2)*sum(w.*quadValues(i,:)'.*quadValues(j,:)'.*quadValues2'*H.stiffness.SVDU{6}{dim}(:,r));
                            end
                        end
                    end
                elseif dim == 2
                    for i = l-space.degree(dim):l
                        for j = l-space.degree(dim):l
                            for r = 1:H.stiffness.R(1,dim)
                                H.stiffness.K{dim}{1}{r}(i,j)  = ...
                                    H.stiffness.K{dim}{1}{r}(i,j)  + ((b-a)/2)*sum(w.*quadValues(i,:)'.*quadValues(j,:)'.*quadValues2'*H.stiffness.SVDU{1}{dim}(:,r));
                            end
                            for r = 1:H.stiffness.R(2,dim)
                                H.stiffness.K{dim}{2}{r}(i,j)  = ...
                                    H.stiffness.K{dim}{2}{r}(i,j)  + ((b-a)/2)*sum(w.*quadValuesDeriv(i,:)'.*quadValues(j,:)'.*quadValues2'*H.stiffness.SVDU{2}{dim}(:,r));
                                H.stiffness.K{dim}{4}{r}(i,j)  = ...
                                    H.stiffness.K{dim}{4}{r}(i,j)  + ((b-a)/2)*sum(w.*quadValues(i,:)'.*quadValuesDeriv(j,:)'.*quadValues2'*H.stiffness.SVDU{2}{dim}(:,r));
                            end
                            for r = 1:H.stiffness.R(3,dim)
                                H.stiffness.K{dim}{3}{r}(i,j)  = ...
                                    H.stiffness.K{dim}{3}{r}(i,j)  + ((b-a)/2)*sum(w.*quadValues(i,:)'.*quadValues(j,:)'.*quadValues2'*H.stiffness.SVDU{3}{dim}(:,r));
                                H.stiffness.K{dim}{7}{r}(i,j)  = ...
                                    H.stiffness.K{dim}{7}{r}(i,j)  + ((b-a)/2)*sum(w.*quadValues(i,:)'.*quadValues(j,:)'.*quadValues2'*H.stiffness.SVDU{3}{dim}(:,r));
                            end
                            for r = 1:H.stiffness.R(4,dim)
                                H.stiffness.K{dim}{5}{r}(i,j)  = ...
                                    H.stiffness.K{dim}{5}{r}(i,j)  + ((b-a)/2)*sum(w.*quadValuesDeriv(i,:)'.*quadValuesDeriv(j,:)'.*quadValues2'*H.stiffness.SVDU{4}{dim}(:,r));
                            end
                            for r = 1:H.stiffness.R(5,dim)
                                H.stiffness.K{dim}{6}{r}(i,j)  = ...
                                    H.stiffness.K{dim}{6}{r}(i,j)  + ((b-a)/2)*sum(w.*quadValues(i,:)'.*quadValuesDeriv(j,:)'.*quadValues2'*H.stiffness.SVDU{5}{dim}(:,r));
                                H.stiffness.K{dim}{8}{r}(i,j)  = ...
                                    H.stiffness.K{dim}{8}{r}(i,j)  + ((b-a)/2)*sum(w.*quadValuesDeriv(i,:)'.*quadValues(j,:)'.*quadValues2'*H.stiffness.SVDU{5}{dim}(:,r));
                            end
                            for r = 1:H.stiffness.R(6,dim)
                                H.stiffness.K{dim}{9}{r}(i,j)  = ...
                                    H.stiffness.K{dim}{9}{r}(i,j)  + ((b-a)/2)*sum(w.*quadValues(i,:)'.*quadValues(j,:)'.*quadValues2'*H.stiffness.SVDU{6}{dim}(:,r));
                ...
                elseif dim == 3
                    for i = l-space.degree(dim):l
                        for j = l-space.degree(dim):l
                            ...
                            for r = 1:H.stiffness.R(2,dim)
                                H.stiffness.K{dim}{2}{r}(i,j)  = ...
                                    H.stiffness.K{dim}{2}{r}(i,j)  + ((b-a)/2)*sum(w.*quadValues(i,:)'.*quadValues(j,:)'.*quadValues2'*H.stiffness.SVDU{2}{dim}(:,r));
                                H.stiffness.K{dim}{4}{r}(i,j)  = ...
                                    H.stiffness.K{dim}{4}{r}(i,j)  + ((b-a)/2)*sum(w.*quadValues(i,:)'.*quadValues(j,:)'.*quadValues2'*H.stiffness.SVDU{2}{dim}(:,r));
                            end
                            for r = 1:H.stiffness.R(3,dim)
                                H.stiffness.K{dim}{3}{r}(i,j)  = ...
                                    H.stiffness.K{dim}{3}{r}(i,j)  + ((b-a)/2)*sum(w.*quadValuesDeriv(i,:)'.*quadValues(j,:)'.*quadValues2'*H.stiffness.SVDU{3}{dim}(:,r));
                                H.stiffness.K{dim}{7}{r}(i,j)  = ...
                                    H.stiffness.K{dim}{7}{r}(i,j)  + ((b-a)/2)*sum(w.*quadValues(i,:)'.*quadValuesDeriv(j,:)'.*quadValues2'*H.stiffness.SVDU{3}{dim}(:,r));
                            end
                            for r = 1:H.stiffness.R(4,dim)
                                H.stiffness.K{dim}{5}{r}(i,j)  = ...
                                    H.stiffness.K{dim}{5}{r}(i,j)  + ((b-a)/2)*sum(w.*quadValues(i,:)'.*quadValues(j,:)'.*quadValues2'*H.stiffness.SVDU{4}{dim}(:,r));
                            ...
                            for r = 1:H.stiffness.R(5,dim)
                                H.stiffness.K{dim}{6}{r}(i,j)  = ...
                                    H.stiffness.K{dim}{6}{r}(i,j)  + ((b-a)/2)*sum(w.*quadValuesDeriv(i,:)'.*quadValues(j,:)'.*quadValues2'*H.stiffness.SVDU{5}{dim}(:,r));
                                H.stiffness.K{dim}{8}{r}(i,j)  = ...
                                    H.stiffness.K{dim}{8}{r}(i,j)  + ((b-a)/2)*sum(w.*quadValues(i,:)'.*quadValuesDeriv(j,:)'.*quadValues2'*H.stiffness.SVDU{5}{dim}(:,r));
                            end
                            for r = 1:H.stiffness.R(6,dim)
                                H.stiffness.K{dim}{9}{r}(i,j)  = ...
                                    H.stiffness.K{dim}{9}{r}(i,j)  + ((b-a)/2)*sum(w.*quadValuesDeriv(i,:)'.*quadValuesDeriv(j,:)'.*quadValues2'*H.stiffness.SVDU{6}{dim}(:,r));
                            end
                        end
                    end
                end
